...
describe("File", () => {
  ...
  describe("File Service", () => {
    ...
    //     // Explicitly await the connection here
    //     await pool.connect();
    ...
    //     // Tests that the function successfully inserts a file into the database and logs the history
    ...
    //       await uploadFileToDatabase('filename.txt', 'fileUrl', 'mediaType', 1);
    //       console.log('Upload')
    ...
    //     // Tests that the function rolls back the transaction if there is an error during the insertion into the files table
    ...
    //       await expect(uploadFileToDatabase('filename.txt', 'fileUrl', 'mediaType', 1)).rejects.toThrow('DB Error');

    //       expect(mockClient.query).toHaveBeenCalledWith('BEGIN');
    //       expect(mockClient.query).toHaveBeenCalledWith('ROLLBACK');
    //     });

    //     // Tests that the function rolls back the transaction if there is an error during the insertion into the fileHistory table
    ...
    //     // Tests that the function uses a transaction for the database operations
    ...
    //     // Tests that the function throws an error if there is an error during the database operations
